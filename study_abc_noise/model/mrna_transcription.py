...
class MRNATranscriptionModelVars(ModelVars):
    ...
    def get_kernel(self):
        if self.noise_model == 'binom':
            ...
            kernel = pyabc.distance.PoissonKernel(
                keys=['mrna'],
                ret_scale=pyabc.distance.SCALE_LOG)
        return kernel
...
class MRNATranscription1dModelVars(MRNATranscriptionModelVars):
    ...
    def __init__(self,
                 ...
                 n_t : int = 30,
                 t_max: int = 90,
                 noise_model: str = 'binom'):
        super().__init__(
            n_acc=n_acc, noise_success_probability=noise_success_probability,
            pdf_max=pdf_max, n_t=n_t, t_max=t_max, noise_model=noise_model)
        self.p_true = OrderedDict([('transcription', 10)])
        self.limits = OrderedDict([('transcription', (0, 30))])
