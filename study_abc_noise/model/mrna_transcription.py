...
class MRNATranscriptionModelVars(ModelVars):
    ...
    def __init__(self):
        super().__init__(
            p_true = OrderedDict([('translation', 10),
                                  ('decay', 0.1),]))
    ...
    def get_id(self):
        return f"cr_{self.n_t}_{self.noise_success_probability}"
    ...
    def get_model(self):
        def model(p):
            ...
            return {'t': result.list_ts[0],
                    'mrna': result.list_xs[0]}
        return model
    
    def get_model_noisy(self):
        def model_noisy(p):
            y = self.get_model()(p)
            successes = np.random.binomial(n=y['mrna'], p=self.noise_success_probability
        return model_noisy
