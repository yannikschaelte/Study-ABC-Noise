...
class OneStateGeneExpressionModelVars(ModelVars):

    def __init__(self):
        super().__init__(
            p_true = OrderedDict([('mRNA translation', 3),
                                  ('mRNA decay', 0.1),
                                  ('mRNA transcription', 0.2),
                                  ('Protein decay', 0.05)]))
        self.limits = {'p0': (0, 0.4), 'p1': (0, 0.4)}
        self.reactants = np.array([[0, 0], [1, 0], [1, 0], [0, 1]])
        self.products = np.array([[1, 0], [0, 0], [1, 1], [0, 0]])
        self.n_t = 100
        self.t_max = 100
